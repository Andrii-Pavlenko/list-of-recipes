...
const RowsOfRecipes = ({ recipe, load, removedIds, removeRecipe, showEditRecipeForm, showAllRecipes }) => {
  ...
  return (
    <Table.Row key={recipe.id}>
      <Table.Cell>{Array.isArray(recipe.title) ? recipe.title[0] : recipe.title}</Table.Cell>
      <Table.Cell className="text-area">{Array.isArray(recipe.description) ? recipe.description[0] : recipe.description}</Table.Cell>
      <Table.Cell>{Array.isArray(recipe["creation_date"]) ? new Date(recipe["creation_date"][0]).toLocaleDateString('uk-UA') : new Date(recipe["creation_date"]).toLocaleDateString('uk-UA')}</Table.Cell>
      <Table.Cell>
